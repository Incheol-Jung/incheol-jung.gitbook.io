...
## MYSQL 에서는 Phantom read가 발생하지 않는다??
...
* 결론은 아니다!! MVCC로는 `Phantom read`를 해결할 수 없다
* 트랜잭션내에서 스냅샷을 생성한다고 하더라도 스냅샷 범위 이외에 변경되는 트랜잭션은 보장하지는 않는다
* 그래서 대부분의 데이터베이스는 격리수준을 `serializable`로 수정해서 해결할 수 있다
* 그러나 `mysql`의 `innoDB`는 스냅샷을 생성할 때, next-key lock을 사용하여 생성/삭제 되는 데이터를 미연에 예방하도록 하여 Phantom read를 해결할 수 있는 것이다

## 그럼 우리는 동시성 제어를 고려하지 않아도 될까?

* 아니다. 이전까지 살펴본건 데이터베이스 내에서 격리수준에 따라 독립성을 보장하는 과정을 살펴봤다
* 서비스를 구현하다보면 서비스 성능을 보장하면서 동시성 제어를 직접 구현해야하는 경우가 있다 `(ex. 재고차감, 선착순 모집, 파격세일 주문)`
