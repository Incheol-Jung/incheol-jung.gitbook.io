...
* 부분적으로는 맞다고 할수 있다. MVCC로는 `Phantom read`를 어느정도는 해결할 수 있다
* 트랜잭션내에서 스냅샷을 생성하여 그 이후 트랜잭션의 내용은 반영하지 않고 스냅샷을 통해서 확인하기 때문에 새로 생성되거나 삭제된 데이터는 확인할 수 없다
* 하지만 부분적으로 맞다고 한 이유는 MVCC 매커니즘으로 동작하는 모든 데이터베이스에서 명시적으로 트랜잭션내 lock을 생성했을 경우에는 데이터 생성/삭제 커밋을 제한하지 않는다
* 그러나 mysql의 innoDB는 트랜잭션 내에서 락을 생성하였다면 next-key lock을 사용하여 조회 범위 다음 인덱스까지 gap 락으로 포함하여 생성/삭제 되는 데이터를 미연에 예방하도록 할 수 있다
