...
불변 컬렉션에서 containts\(\) 메서드를 서술적으로 사용하는 것이 명령적 스타일을 사용하는 경우보다 코드를 작성하는 면에서 얼마나 쉬운지 알아봤다. 이터이션을 제어하는 것과 같은 노력이 많이 들고 주의 깊게 작성해야 하는 코드와 로우 레벨의 코드들은 라이브러리의 함수들을 사용하여 처리하도록 하고 그 세부적인 코드까지는 신경 쓸 필요가 없다.
